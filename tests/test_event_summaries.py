...
def test_redeem_voucher_event_summary_init():
    SYSTEM_NAME = "HR 1597"
    MINOR_FACTION = "HR 1597 & Co"
    SUPPORTS = True
    VOUCHER_TYPE = "bounty"
    AMOUNT = 512546
    redeem_voucher_event_summary = RedeemVoucherEventSummary(SYSTEM_NAME, MINOR_FACTION, SUPPORTS, VOUCHER_TYPE, AMOUNT)
    assert(redeem_voucher_event_summary.system_name == SYSTEM_NAME)
    assert(redeem_voucher_event_summary.minor_faction == MINOR_FACTION)
...
def test_sell_exploration_data_event_summary_init():
    ...
    MINOR_FACTION = "Anti Energy Company"
    SUPPORTS:bool = True
    AMOUNT:int = 512546
    sell_exploration_data_event_summary:SellExplorationDataEventSummary = SellExplorationDataEventSummary(SYSTEM_NAME, MINOR_FACTION, SUPPORTS, AMOUNT)
    assert(sell_exploration_data_event_summary.system_name == SYSTEM_NAME)
    assert(sell_exploration_data_event_summary.minor_faction == MINOR_FACTION)
...
def test_market_sell_event_summary_init():
    ...
    MINOR_FACTION = "Hutton Orbital Truckers"
    SUPPORTS:bool = True
    COUNT:int = 100
    SELL_PRICE_PER_UNIT:int = 10
    AVERAGE_BUY_PRICE_PER_UNIT:int = 5
    market_sell_event_summary:MarketSellEventSummary = MarketSellEventSummary(SYSTEM_NAME, MINOR_FACTION, SUPPORTS, COUNT, SELL_PRICE_PER_UNIT, AVERAGE_BUY_PRICE_PER_UNIT)
    assert(market_sell_event_summary.system_name == SYSTEM_NAME)
    assert(market_sell_event_summary.minor_faction == MINOR_FACTION)
...
def test_mission_completed_event_summary_init():
    SYSTEM_NAME = "Sol"
    MINOR_FACTION = "Mother Gaia"
    SUPPORTS = True
    INFLUENCE = "++"
    mission_completed_event_summary = MissionCompletedEventSummary(SYSTEM_NAME, MINOR_FACTION, SUPPORTS, INFLUENCE)
    assert(mission_completed_event_summary.system_name == SYSTEM_NAME)
    assert(mission_completed_event_summary.minor_faction == MINOR_FACTION)
