...
this.activity:tk.StringVar = tk.StringVar()
...
def plugin_app(parent: tk.Frame) -> Union[tk.Widget, Tuple[tk.Widget, tk.Widget]]:
    ...
    tk.Label(frame, textvariable=this.activity, anchor=tk.W, justify=tk.LEFT, pady=10).grid(row=1, column=0, columnspan=2, sticky=tk.W)
...
def copy_activity_to_clipboard() -> None:
    ...
    root.clipboard_append(this.activity.string)
    root.update()
    root.destroy()

def clear_activity() -> None:
    this.activity.set("(No activity)")